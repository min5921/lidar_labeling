...
11. 설치된 Python에 우연히 의존하지 않도록 고정 lock 기반 가상환경을 Windows와 Linux에서 검증한다.
12. 저장소 기본 설정은 읽기 전용으로 취급하고 사용자 설정·로그는 사용자 쓰기 가능 경로에 둔다.
...
- Windows와 Linux의 깨끗한 Python 3.10+ 가상환경에서 설치되고 한글/공백 경로의 데이터셋을 열 수 있다.
