...
- 선택 객체의 다음 frame 자동 추적은 별도로 켜는 시험 기능입니다. 크기·yaw를 유지하며 x/y와
  선택적 z만 조정합니다. 지면 보정은 차량·사람용으로 객체별로 켜고, 공중 표지판은 기본
  포인트 이동량 방식을 사용합니다. 불확실한 추적은 이전 위치를 유지하며 Undo로 취소할 수 있습니다.
- 자동 추적 ON에서는 선택 객체만 다음 frame으로 이어가므로 다른 객체의 미편집 사본이 미리
  저장되지 않습니다. 이미 같은 ID가 저장되어 있다면 `선택 객체: 기존 박스도 다시 추적`을
  해당 객체에만 켤 수 있습니다. 기본은 기존 위치를 보존하며 검토 완료/건너뜀·복구 라벨은 보호합니다.
- 선택 박스 내부의 포인트는 노란색으로 강조됩니다. 표시 옵션에서 끌 수 있습니다.
