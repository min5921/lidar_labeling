...
단, 아래 자동 추적을 켜면 **선택 객체 하나만** 복사한다. 선택이 없으면 복사하지 않는다.
...
3. `다음 ▶` 또는 `→`를 누르면 선택 객체만 다음 포인트에 맞춰 자동 이동을 시도한다.
   이 경우 선택한 원본 import 객체도 같은 ID로 이어받을 수 있다. 나머지 객체는 새로 복사하지
   않으므로 A 객체를 끝까지 작업한 뒤 처음으로 돌아와 B 객체를 따로 이어갈 수 있다.
4. 결과를 확인하고 필요하면 수동 조정한다. Ctrl+Z 한 번이면 자동 이동 전 위치로 돌아간다.

추적은 기본 OFF다. 검색 범위는 `추적 최대 이동`에서 조절하며 기본 반경은 4 m다. 크기와 yaw는
자동으로 바꾸지 않는다. 대상에 이미 같은 ID가 있으면 기본적으로 기존 라벨을 유지한다. 이전/건너뛴 프레임과
폴더 경계에서는 추적하지 않는다. 포인트 부족·가림·비슷한 후보·급격한 이동 등에서는 이전 위치를
유지하고 상태줄에 이유를 표시한다. 학습형 자동 추적기가 아닌 **프레임별 작업 보조**이므로 검수가 필요하다.

이미 다음 프레임에 자동 복사된 박스가 저장되어 추적이 멈춘 경우:

1. 바로 이전 프레임에서 해당 객체 위치를 맞추고 선택한다.
2. `선택 객체: 기존 박스도 다시 추적`을 켜고 `다음 ▶` 또는 `→`로 이동한다.
3. 같은 ID·클래스의 기존 박스 위치를 다시 계산한다. 대상의 크기·회전·속성 및 다른 객체는
   유지하며, z 조정이 꺼져 있으면 대상 z도 그대로 둔다. Ctrl+Z 한 번으로 재추적 전으로 돌아간다.

이 옵션은 **기본 OFF·현재 실행의 해당 객체 ID에만 적용**된다. 다른 객체를 선택하면 그 객체의
설정이 표시된다. 자동 복사본인지 수동 편집본인지 판별하는 기능이 아니므로, 켠 객체의 미완료
프레임에서 수동 조정한 위치도 달라질 수 있다. 검토 완료/건너뜀 프레임의 기존 박스와 복구한
라벨은 보호한다. 완료된 프레임을 의도적으로 다시 작업하려면 그 프레임에서
`재검토: 작업 중으로 변경` 후 저장하고, 이전 프레임으로 돌아와 다시 추적한다.
기존 파일을 지우거나 1000개 프레임의 라벨을 일괄 삭제할 필요는 없다.
...
- 차량·사람처럼 지면에 붙은 객체는 `선택 객체: 지면 높이로 z 보정`을 추가로 켠다.
  다음 프레임 추적 때 주변 지면을 확인하고, `B`와 동일한 박스 footprint 하위 5% 포인트 높이로
  바닥을 맞춘다. 같은 포인트·박스 XY 위치에서는 자동 보정 뒤 `B`를 눌러도 높이가 다시 내려가지
  않는다. 지면 근거가 부족하거나 내부 포인트 높이가 주변 지면과 맞지 않으면 z는 유지한다.
...
7. `만든/가져온 박스 다음 프레임으로 이어가기`를 켜 두면 이후 프레임에도 같은 ID로 이어진다.
   객체 하나씩 구간을 작업하려면 자동 추적을 켠다. 이때 선택 객체만 이어가고 나머지는 첫 프레임에
   남아 있으므로, 다음 객체를 작업할 때 처음으로 돌아가 그 객체를 선택하면 된다.
...
`Ctrl` 하강은 Ctrl만 눌렀다 놓을 때 이동 step만큼 한 번 실행된다. 누른 채 유지하는 동안
연속으로 내려가지는 않는다. 마우스 버튼을 누르지 않고 커서만 움직이는 것은 괜찮다.
Ctrl과 클릭·드래그·휠 또는 Shift·Alt 같은 보조 키를 함께 사용하면 하강하지 않는다.
`Ctrl+S`, `Ctrl+Z`, `Ctrl+Y` 같은 조합 단축키에 하강 동작이 추가로 실행되지는 않는다.
...
Frame 패널의 `검토 완료`, `건너뜀`, `재검토: 작업 중으로 변경` 버튼은 현재 프레임의 상태를 명시적으로 바꾼다.
박스가 존재하거나 자동 추적에 성공했다는 이유로 검토 완료가 되지는 않는다. 완료·건너뜀 뒤
객체를 편집하면 다시 작업 중이 된다. 상태 변경도 Undo/Redo가 가능하며 일반 저장을 해야
디스크에 확정된다.

`상태 갱신`으로 활성 profile 전체의 상태를 읽고 필터로 미검토/작업 중/완료/건너뜀을
