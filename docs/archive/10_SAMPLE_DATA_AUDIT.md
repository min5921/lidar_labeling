...
보관 이력 문서입니다. [현행 유지보수 결과](../35_PROJECT_REVIEW_AND_MAINTENANCE.md)를 먼저 확인하세요.

> 이 문서는 초기 Waymo-style 샘플 한 건의 읽기 전용 검사 기록이다. 범용 v2 입력 형식이나
> 현재 사용자 설정 절차를 정의하지 않는다. 새 데이터 구성은
> `docs/33_GENERIC_DATASET_SETUP_GUIDE.md`를 따른다.
