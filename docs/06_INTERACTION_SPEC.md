...
- `선택 박스 안 포인트 노란색 강조` 기본 ON. yaw와 z 범위를 포함한 3D 내부 판정을 사용하며
  전체/상세 3D·BEV·측면에서 같은 의미를 유지한다. 선택 해제·박스 이동·옵션 OFF 시 즉시 복원한다.
  렌더 포인트만 강조하고 표시 캐시의 기본 색이나 원본 배열은 변경하지 않는다.
...
Ctrl+Z, Ctrl+Y 등 기존 조합 단축키에 하강 동작이 추가로 실행되지는 않는다.
한 번 놓을 때 이동 step만큼 한 번만 내리며, 버튼을 누르지 않은 단순 마우스 이동은
입력을 취소하지 않는다. Ctrl과 클릭·드래그·휠 또는 다른 modifier를 함께 사용하거나
창을 벗어나면 하강하지 않는다.
...
6. 순차적으로 다음 프레임을 열 때 옵션이 켜져 있으면 도구에서 생성했거나 사용자가 `이전 폴더의 객체 가져오기…`로 가져온 객체를 같은 ID로 이어받는다. 그 밖의 원본 import 객체는 자동 복제하지 않는다.
   단, 사용자가 자동 추적을 켠 **선택 객체 하나**는 원본 import 객체여도 명시적 이어받기 대상이다.
...
- `선택 객체 다음 프레임 자동 추적 (시험 기능)`은 기본 OFF이며 이어가기 옵션과 함께 켠다.
- 다음 버튼/→로 바로 다음 frame에 이동할 때만 선택 객체를 추적한다. 나머지 객체는 기존
  이어받기 정책을 유지한다. 이전/콤보 이동, 폴더 간 가져오기는 자동 추적하지 않는다.
- 같은 단일 LiDAR·reference frame의 원본 포인트에서 객체 부분을 읽고 제한된 표본으로
  국소 이동량을 비교한다. 화면 다운샘플/색/센서 표시 품질은 추적 계산의 표본 설정이 아니다.
- 최대 수평 이동은 기본 4 m(0.5~10 m 조절), 자동 z 변경은 최대 0.6 m다. 크기와 yaw는 유지한다.
- `추적 시 상하 위치(z)도 조정 · 크기 유지` 기본 ON: 객체 포인트의 z 이동량을 적용하므로
  공중 표지판을 지면에 붙이지 않는다. 끄면 z를 유지한다.
- `선택 객체: 지면 높이로 z 보정` 기본 OFF: 현재 실행의 객체 ID별 선택이다. 선택을 바꾸면
  그 객체의 설정을 표시한다. 켠 객체만 주변 지면을 추정하며 충분한 외부 지면 지지가 없으면 z를 유지한다.
- 대상 frame에 같은 ID가 있거나 복구본을 복원하면 자동 위치 변경을 적용하지 않는다.
- 포인트 부족, 유사 후보, 낮은 신뢰도, 이동 한도, 계산 오류는 이전 위치를 유지하고 이유를 표시한다.
- 추적은 frame worker에서 계산하고 generation/객체/출발·도착 frame이 맞을 때만 적용한다.
  로드 중 편집은 막고 취소·다른 요청·종료 시 추적 요청을 취소한다.
- 일반 이어받기와 자동 이동은 Undo 단계가 다르다. Ctrl+Z 한 번이면 자동 이동 전 박스로
  돌아간다. 추적만으로 frame을 reviewed 처리하거나 별도 파일을 자동 덮어쓰지 않는다.
