...
6. 순차적으로 다음 프레임을 열 때 옵션이 켜져 있으면 도구에서 생성했거나 사용자가 `이전 폴더의 객체 가져오기…`로 가져온 객체를 같은 ID로 이어받는다. 그 밖의 원본 import 객체는 자동 복제하지 않는다.
   자동 추적 ON에서는 **선택 객체 하나만** 이어받고, 원본 import 객체도 선택했다면 대상이다.
   선택이 없으면 복사하지 않는다. 자동 추적 OFF에서만 만든/가져온 박스 전체를 이어간다.
...
- 다음 버튼/→로 바로 다음 frame에 이동할 때만 선택 객체를 이어받아 추적한다. 나머지 객체는
  새로 복사하지 않으며 대상에 이미 있는 객체는 유지한다. 이전/콤보 이동, 폴더 간 가져오기는
  자동 추적하지 않는다.
...
- 대상 frame에 같은 ID가 있으면 기본적으로 위치를 유지한다. `선택 객체: 기존 박스도 다시
  추적`을 켠 ID만 같은 클래스의 미완료 기존 박스를 재추적한다. 대상의 크기·yaw·속성은 유지하며
  z 조정 OFF이면 대상 z도 유지한다. 기본 OFF·현재 실행 한정이며 다른 객체 선택에 전파하지 않는다.
- 검토 완료/건너뜀 프레임의 기존 박스, 복구본을 복원한 프레임, 계산 후 target snapshot이 바뀐
  경우는 재추적하지 않는다. 사용자가 완료 상태를 해제하려면 기존 `재검토: 작업 중으로 변경`을
  사용한다. 다른 frame의 파일을 삭제하거나 기존 사본을 일괄 정리하지 않는다.
...
- 일반 이어받기와 자동 이동은 Undo 단계가 다르다. Ctrl+Z 한 번이면 자동 이동 전 박스로
  돌아간다. 기존 박스 재추적은 복사 단계 없이 한 번의 Undo로 원래 박스를 복원한다.
  추적만으로 frame을 reviewed 처리하거나 별도 파일을 자동 덮어쓰지 않는다.
...
- 가져온 객체는 `만든/가져온 박스 다음 프레임으로 이어가기`가 켜져 있으면 다음 프레임으로
  이어진다. 자동 추적 OFF이면 모두 복사하고 ON이면 선택 객체만 복사·추적한다.
  폴더 사이 가져오기 자체에는 위치 예측이나 좌표 변환이 없으므로 처음 위치를 직접 확인한다.
