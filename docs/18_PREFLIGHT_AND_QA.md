...
- working label에 기록된 source/calibration fingerprint와 현재 파일의 불일치
...
GUI는 같은 포인트 클라우드, 다운샘플 한도, 색상 조건을 다시 표시할 때 렌더링 전용
XYZ/RGBA 배열을 제한된 LRU 캐시에서 재사용한다. 캐시는 표시 계산에만 관여하며 원본
`PointCloudData` 배열이나 BIN/PCD 파일을 수정하지 않는다. 객체 선택, 박스 선 두께 변경,
카메라 overlay 변경은 포인트 클라우드를 다시 계산하지 않는다. 측면 평면 변경은 측면 뷰만
다시 투영한다.

화면이 느리면 다음 순서로 확인한다.

1. 사용하지 않는 LiDAR 센서를 끈다.
2. `configs/default.json`의 `point_cloud.max_render_points`를 낮춘다.
3. intensity/height 색상 대신 sensor 또는 uniform 색상을 사용한다.
4. BEV·측면 보조 뷰를 필요한 동안만 연다.
5. 상태 표시줄 tooltip에서 활성 센서, 표시 포인트 수와 렌더 캐시 사용량을 확인한다.
