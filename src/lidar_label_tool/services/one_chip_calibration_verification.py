...
def _representative_frame_ids(dataset: Path) -> tuple[str, ...]:
    frame_ids = DeviceCentricAdapter(dataset).scan().frame_ids
    if not frame_ids:
        return ()
    indexes = sorted({0, len(frame_ids) // 4, len(frame_ids) // 2, 3 * len(frame_ids) // 4, len(frame_ids) - 1})
    return tuple(frame_ids[index] for index in indexes)
...
def verify_calibration(
    ...
    *,
    ...
    calibration_path = dataset / "calibration" / "calibration.json"
    ...
    projections, projection_issues = _projection_stats(
        dataset,
        tuple(frames) or _representative_frame_ids(dataset),
        sample_step=sample_step,
        write_overlays=write_overlays,
        overlay_dir=overlay_dir,
        overlay_max_points=overlay_max_points,
    )
    ...
    return CalibrationVerificationResult(report, tuple(issues), report_output, overlay_dir)
...
def verify(args: argparse.Namespace) -> int:
    ...
    calibration_path = Path(str(report["calibration_path"]))
    current_metadata = report.get("metadata", {})
    comparison = report.get("comparison", {})
    projections = report.get("projection_stats", [])

    print(f"Calibration: {calibration_path}")
    print(f"Source YAML: {args.calibration_root}")
    print(f"raw_extrinsic_convention: {current_metadata.get('raw_extrinsic_convention')}")
    print(f"camera_frame_convention: {current_metadata.get('camera_frame_convention')}")
